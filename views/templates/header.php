<header>
	<div id="menu_animacion" class="header">
		<div class="menu">
			<div class="responsive_menu responsive_movil_on">
				<div class="a"></div>
				<div class="b"></div>
				<div class="c"></div>
			</div>
			<div class="info_empresa responsive_movil_on">
				<span onclick="window.location = '<?php echo HOST?>contacto'">
					<svg viewBox="0 0 512.000000 362.000000"
					 preserveAspectRatio="xMidYMid meet">
					<g transform="translate(0.000000,362.000000) scale(0.100000,-0.100000)"
					fill="#FFF" stroke="none">
					<path d="M340 3604 c-30 -8 -59 -19 -65 -25 -11 -11 2161 -2190 2207 -2215 35
					-18 121 -18 156 0 15 8 520 508 1122 1110 816 817 1092 1098 1083 1107 -39 39
					-45 39 -2288 38 -1871 0 -2167 -2 -2215 -15z"/>
					<path d="M23 3308 c-17 -50 -18 -123 -18 -1488 0 -1365 1 -1438 18 -1487 10
					-29 22 -53 25 -53 4 0 354 347 777 770 l770 770 -770 770 c-423 424 -773 770
					-777 770 -3 0 -15 -24 -25 -52z"/>
					<path d="M4295 2590 l-770 -770 770 -770 c423 -423 773 -770 777 -770 3 0 15
					24 25 53 17 49 18 122 18 1487 0 1365 -1 1438 -18 1488 -10 28 -22 52 -25 52
					-4 0 -354 -346 -777 -770z"/>
					<path d="M1030 835 c-495 -495 -761 -768 -755 -774 6 -6 37 -17 70 -25 53 -14
					299 -16 2215 -16 2237 0 2244 0 2283 39 9 9 -183 206 -753 776 -421 421 -768
					765 -772 765 -4 0 -110 -103 -235 -229 -231 -232 -273 -265 -378 -301 -70 -24
					-206 -27 -276 -6 -111 34 -148 62 -390 302 -129 129 -237 234 -240 234 -2 0
					-348 -344 -769 -765z"/>
					</g>
					</svg>
				</span>
			</div>
			<div class="lateral_izq">
				<div class="main_titulo" onclick="window.location = '<?php echo HOST?>inicio'">
					<img src="<?php echo IMG?>pcshop_a.png" alt="logo" />
					<span>PC SHOP</span>
				</div>
				<nav class="nav">
					<a class="nav_elemento <?php echo $inicio?>" href="<?php echo HOST?>inicio">Productos</a>
					<a class="nav_elemento <?php echo $nosotros?>" href="<?php echo HOST?>quienes_somos">Nosotros</a>
					<a class="nav_elemento <?php echo $contacto?>" href="<?php echo HOST?>contacto">Contacto</a>
				</nav>
			</div>
			<div class="lateral_der">
				<!-- <div class="separador responsive_movil_off"></div> -->
				<div class="info_empresa responsive_movil_off">
					<!-- <span>
						<svg viewBox="0 0 512.000000 512.000000"
						 preserveAspectRatio="xMidYMid meet">
						<g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)"
						stroke="none">
						<path d="M283 5106 c-103 -33 -204 -122 -251 -224 l-27 -57 1 -250 c0 -257 12
						-398 55 -650 226 -1344 1020 -2516 2194 -3240 171 -105 522 -282 705 -354 557
						-220 1176 -340 1700 -329 163 3 166 3 231 35 80 40 159 120 197 200 l27 58 0
						585 0 585 -27 58 c-50 107 -161 201 -268 226 -25 6 -133 15 -240 21 -278 15
						-492 51 -719 122 -124 38 -216 46 -301 24 -83 -21 -91 -27 -448 -298 l-299
						-226 -114 65 c-548 314 -920 683 -1230 1222 l-78 133 227 301 c125 165 237
						314 250 331 34 46 62 148 62 225 0 48 -10 103 -35 187 -78 263 -111 465 -125
						754 -8 186 -15 223 -58 306 -30 59 -109 131 -182 167 l-65 32 -570 2 c-452 2
						-579 -1 -612 -11z"/>
						</g>
						</svg>
						(+57) 322 6674241
					</span> -->
					<span>
						<svg viewBox="0 0 512.000000 362.000000"
						 preserveAspectRatio="xMidYMid meet">
						<g transform="translate(0.000000,362.000000) scale(0.100000,-0.100000)"
						stroke="none">
						<path d="M340 3604 c-30 -8 -59 -19 -65 -25 -11 -11 2161 -2190 2207 -2215 35
						-18 121 -18 156 0 15 8 520 508 1122 1110 816 817 1092 1098 1083 1107 -39 39
						-45 39 -2288 38 -1871 0 -2167 -2 -2215 -15z"/>
						<path d="M23 3308 c-17 -50 -18 -123 -18 -1488 0 -1365 1 -1438 18 -1487 10
						-29 22 -53 25 -53 4 0 354 347 777 770 l770 770 -770 770 c-423 424 -773 770
						-777 770 -3 0 -15 -24 -25 -52z"/>
						<path d="M4295 2590 l-770 -770 770 -770 c423 -423 773 -770 777 -770 3 0 15
						24 25 53 17 49 18 122 18 1487 0 1365 -1 1438 -18 1488 -10 28 -22 52 -25 52
						-4 0 -354 -346 -777 -770z"/>
						<path d="M1030 835 c-495 -495 -761 -768 -755 -774 6 -6 37 -17 70 -25 53 -14
						299 -16 2215 -16 2237 0 2244 0 2283 39 9 9 -183 206 -753 776 -421 421 -768
						765 -772 765 -4 0 -110 -103 -235 -229 -231 -232 -273 -265 -378 -301 -70 -24
						-206 -27 -276 -6 -111 34 -148 62 -390 302 -129 129 -237 234 -240 234 -2 0
						-348 -344 -769 -765z"/>
						</g>
						</svg>
						cotizaciones@tintosoft.com.co
					</span>
				</div>
			</div>
		</div>
	</div>
</header>